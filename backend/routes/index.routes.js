...
router.get("/ping", async (req, res) => {
    try {
        ...
        res.json({ message: "pong", result: result[0] });  // Envía una respuesta JSON con el mensaje "pong" y el resultado de la consulta
    } catch (error) {
        console.error("Error during database query:", error);  // Imprime el error en la consola
        res.status(500).json({ message: "Internal Server Error" });  // Envía una respuesta de error 500
    }
});
